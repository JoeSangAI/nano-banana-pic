...
**密钥：** 使用环境变量 `GOOGLE_API_KEY` 或在本目录下复制 `.env.example` 为 `.env` 填入（勿提交 .env）。
...
参数：`--resolution` 1K|2K|4K，`--aspect-ratio`，`--output`，`--number` 等。详见 Cursor skill：nano-banana-pic。GitHub: [nano-banana-pic](https://github.com/JoeSangAI/nano-banana-pic)
