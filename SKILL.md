...
## Advanced Text Editing Workflow

When you need to modify text within an already generated image while strictly preserving the layout and character avatars (such as changing the title or guest info on a poster):

**Step 1: Extract Text and Metadata**
Extract the text and original generation metadata into a Markdown file:
```bash
python3 tools/nano_banana_pic/extract_text.py --image "output/images/your_poster.png" --reference-images "path/to/avatar1.png"
```

**Step 2: Edit the Markdown File**
Open the generated `.md` file (e.g. `output/images/your_poster.md`) and modify the extracted text under the `# 海报文本内容` section as needed. Keep the YAML frontmatter intact.

**Step 3: Reconstruct the Image**
Generate the new image with the updated text using the edited Markdown file:
```bash
python3 tools/nano_banana_pic/replace_text.py --md-file "output/images/your_poster.md" --resolution 2K
```
The new image will be saved with a `_text_edited.png` suffix.
